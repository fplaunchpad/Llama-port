(* microgpt inference - plain OCaml port.

   Implements the contract in BENCHMARK.md: same weights, same f64 operation order,
   same xorshift64* RNG, same JSON report as the Python, C++, Rust and OxCaml ports.

   Build (stock upstream ocamlopt, no flambda):
     ocamlopt -unsafe -I +unix unix.cmxa main.ml -o build/microgpt_infer

   This file is byte-identical to impl/oxcaml/main.ml apart from this header and the
   two identity strings in the JSON report, and build.sh asserts that it stays that
   way. That is the entire point of having both: the source is held fixed so the
   measurement isolates the COMPILER. impl/oxcaml is built by Jane Street's OxCaml
   variant (flambda2 middle-end, -O3); this one is built by the upstream compiler
   from a plain opam switch (closure middle-end, where -O3 does not exist). Neither
   file uses any OxCaml language extension - no modes, no unboxed types, no locality
   annotations - so both compilers accept both files. See OPTIMIZATIONS.md.
...
let () =
  ...
  (try
     ...
     jstr w "build" "ocamlopt (stock upstream); no fast-math equivalent exists in OCaml";
